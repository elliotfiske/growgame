...
(function(window, document) {
   ...
   // Make an input stack to see what direction we should face
   var Game_inputStack = [];
   ...
   document.onkeydown = function(evt) {
      ...
      if (Game_inputStack.indexOf(evt.keyCode) == -1) {
        Game_inputStack.push(evt.keyCode);
      }
      console.log("Pushed " + evt.keyCode + " stack now " + Game_inputStack);
   };
   document.onkeyup = function(evt) {
      ...
      // Remove key from input stack on keyup
      for (var i = Game_inputStack.length-1; i >= 0; i--) {
         if (Game_inputStack[i] == evt.keyCode) {
            Game_inputStack.splice(i, 1);
         }
      }
   };
   ...
   Game.inputStackTop = function() {
      for (var i = 0; i < Game_inputStack.length; i++) {
          var stackBottom = Game_inputStack[i];
          for (var key in KEYS) {
             if (KEYS[key] == stackBottom) {
                 return key;
             }
          }
      }
